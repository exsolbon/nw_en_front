    ...
    <q-card flat dark>
      <q-card-section class="no-padding">
        ...
        <div class="col-12 col-md-6 q-pl-md-lg">
          ...
          <p v-if="item.tier"> <span class="text-bold" >Tier: {{item.tier | tier}}</span> </p>
      ...
      <q-card-section class="no-padding">
        <p v-if="item.baseDamage" class="q-mb-none"><span class="text-bold text-h6">{{item.baseDamage}}</span> Damage</p>
        <p v-if="item.critChance" class="q-mb-none"><span class="text-bold text-h6">{{item.critChance * 100}}%</span> Critical Hit Chance</p>
        <p v-if="item.critDamageMultiplier" class="q-mb-none"><span class="text-bold text-h6">{{item.critDamageMultiplier}}</span> Critical Damage Multiplier</p>
        ...
        <p v-if="item.blockStability" class="q-mb-none"><span class="text-bold text-h6">{{item.blockStability}}</span> Block Stability</p>
         ...
         <p v-if="item.weight" class="q-mb-none"><span class="text-bold text-h6">{{item.weight/10}}</span> Weight</p>
         <p v-if="item.durability" class="q-mb-none"><span class="text-bold text-h6">{{item.durability}}</span> Durability</p>
         <p class="q-mb-none" v-if="item.bindOnEquip"><span class="text-bold text-h6 text-positive">Bind On Equip</span></p>
         <p class="q-mb-none" v-if="item.bindOnPickup"><span class="text-bold text-h6 text-positive">Bind On Pickup</span></p>
         <p class="q-mb-none" v-if="item.namedItem"><span class="text-bold text-h6 text-positive">Named Item</span></p>
         <p class="q-mb-none" v-if="item.can_be_crafted"><span class="text-bold text-h6 text-positive">Can be crafted</span></p>
         <p class="q-mb-none" v-if="item.quest_reward"><span class="text-bold text-h6 text-positive">Quest reward</span></p>
         <q-separator spaced="lg" dark/>
