...
<div  class="characteristics q-mb-md">
        ...
        <div class="flex items-center justify-between q-mt-md">
           <p class="text-bold text-h6 q-mb-none">Points left: {{can_add}}</p>
          <q-btn color="primary" text-color="dark" @click="resetPoints" label="Reset"/>
        </div>
      </div>
       ...
       <div  class=" q-mb-lg">
        <div v-if="!is_build_saved">
          ...
          <div class="flex justify-between">
            <q-checkbox v-model="build_private" color="red" class="q-mb-sm" dark label="Private build (available only by link and not displayed in the general list)" />
            <q-input style="flex-basis: 49%" v-model="build_name" class="q-mb-md" filled label="Name" dark/>
           <q-select style="flex-basis: 49%" dark v-model="build_purpose" filled :options="options" label="Purpose" />
          </div>
        ...
        verdana: 'Verdana'
          ...
          <q-btn @click="saveBuild" :disable="!build_description || !build_name" :loading="is_loading" color="primary" text-color="dark" no-caps label="Save"/>
        </div>
        <div v-else class="">
          <p class="text-h6 text-bold">Build link: <span class="text-primary  inline-block q-mx-sm">https://www.nwfans.ru/builds/{{build_slug}}</span>
            <span @click="copyCB(`https://www.nwfans.ru/builds/${build_slug}`)" class="text-caption cursor-pointer"
                  style="border-bottom: 1px dashed #fff">{{is_copied ? 'copied' : 'copy'}}</span></p>
        </div>
      </div>
     </div>
...
export default {
  ...
  methods:{
    ...
    async saveBuild(){
      ...
      this.$q.notify({
        message: 'Thank you, the link is active and can be shared with friends',
