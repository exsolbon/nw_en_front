   ...
   <q-tab-panels v-model="panel" animated class="bg-dark">
            ...
            <q-card  flat v-if="!item.is_selected" @click="selectWeapon(index)" v-for="(item,index) in weapons" :key="item.id" class="weapon-card cursor-pointer bg-grey-5 ">
              <q-img :src="item.image" :ratio="16/9">
                <div class="text-h5 absolute-bottom text-right no-padding">
                  ...
                  <p class="text-caption q-mb-none q-mr-sm">Characteristics: {{item.main_char}}</p>
                </div>
              </q-img>
        ...
        <q-tab-panel name="calc">
          <div  class="flex items-center justify-between">
            ...
            <p class="q-mb-none text-h6 text-bold text-primary ">Choose another weapon</p>
            </div>


            <q-btn @click="reset" color="primary" text-color="dark" label="Resetting nested points"/>
          </div>
          ...
          <div  class="skills-wrapper q-mb-lg">
            ...
            <div  class="skill-tree" v-for="(tree,tree_index) in weapon.trees" :key="tree.id">
              ...
              <p class="text-center text-h5 q-mb-sm text-primary text-bold">{{tree.name_en}} </p>

              <p class="text-center text-body1 ">Points invested :
                {{tree_index===0 ? points_spent_left : points_spent_right}} (Available {{avaiable_points}})
              </p>
              ...
              <div :ref="`row${row_index+1}tree${tree_index}`" class="skill-tree__row " v-for="(row,row_index) in 6">
                <div v-if="!item.is_empty"
                  ...
                  >
                    ...
                    <p style="max-width: 350px" class="text-center text-body2">{{item.description_en}}</p>
                  </q-tooltip>
                </div>
